...
public class CommandManager implements CommandExecutor, TabCompleter {
    ...
    private void processRequirements(AbstractCommand command, CommandSender sender, String[] args) {
        ...
        if (command.getPermissionNode() == null || sender.hasPermission(command.getPermissionNode())) {
            ...
            } else if (returnType == AbstractCommand.ReturnType.MISSING_ARGUMENT){
                sender.sendMessage("You're missing an argument/s.");
            }
            return;
        }
        ...
        if (nested != null) {
            ...
            if (args.length == 1) {
                ...
                return nested.children.entrySet().stream()
                        .filter(e -> !console || !e.getValue().isPlayerOnly())
